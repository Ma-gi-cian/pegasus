...
static std::vector<std::string> parse_csr_json(char const* filename)
{
    ...
    std::string primeString((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
...
{
    /*
    This function goes and gets the register values every frame / every execution and then compares
    it with the register values before hand - and assigns true false to the boolean register changed
    so that we can do color changes to that particular row.
    */
    s.reg_changed = {};
    for (int i = 0; i < 32; i++)
    {
        std::vector<uint8_t> buf(8, 0);
        // buff needs to be a vector because down the line a resize operation is being performed in
        // it and well the function signature is that of vector so
        try
        {
...
static void initialize(SimState & s)
{
    ...
    try
    {
        std::cout << "The ilimit is : " << s.ilimit << " file path " << s.file_path
                  << "params : " << s.params.size() << std::endl;
        s.cosim = new pegasus::cosim::PegasusCoSim(s.ilimit, s.file_path, s.params,
                                                   "pegasus-cosim.db", 100);

        printf("It did not reach here");
        s.running = true;
        s.finished = false;
        s.total_step = 0;
        s.total_commited = 0;
...
static void walk(SimState & s)
{
    ...
    try
    {
        ...
        // evt has euid - the euid does not change it always remains the same - but the status of
        // the event assosiated with the eventaccessor will change hence we can do the below thing.
    ...
    catch (std::exception & e)
    {
        ...
        s.step_automatically =
            false; // Incase the entire thing is running automatically and an error occurs - we dont
                   // want the simulator to keep on trying to run itself - we just want it to pause
        s.error_occured = true;
    }
}
...
static void drawEventLog(SimState & s, float x, float y, float W, float H)
{
    ...
    if (ImGui::Begin("Instruction Log", nullptr, flags))
    {
                              ...
                              ImVec2(0, H - 40)))
        {
            ...
            ImGui::TableSetupColumn("rRegs", ImGuiTableColumnFlags_WidthFixed,
                                    45); // register reads
            ImGui::TableSetupColumn("wRegs", ImGuiTableColumnFlags_WidthFixed,
                                    45);                                          // register writes
            ...
            for (auto it = s.logs.rbegin(); it != s.logs.rend(); ++it)
            {
                ...
                if (e.faulted)
                    row_col = {1.0f, 0.4f, 0.4f, 1.0f}; // red
                else if (e.is_branch)
                    row_col = {1.0f, 0.85f, 0.4f, 1.0f}; // orange
                ...
                if (e.has_reg_writes)
                {
                    ...
                    if (ImGui::IsItemHovered() && !e.reg_write_summary.empty())
                    {
                        ...
                        {
                            ImGui::Text("%s = %s", rw.first.c_str(), hex64(rw.second).c_str());
                        }
                        ImGui::EndTooltip();
                    }
                }
    ...
    if (ImGui::Begin("CSR File", nullptr, flags))
    {
                              ...
                              ImVec2(0, H - 40)))
        {
            ...
            for (int idx : order)
            {
                ...
                ImVec4 col = s.csr_changed[idx] ? ImVec4{1.0f, 1.0f, 0.2f, 1.0f}
...
// currently I still need to add the method of taking a input in the header of the number of steps
// to walk and then walking the steps
static void drawToolbar(SimState & s, float W, float H)
{
    ...
    if (ImGui::Begin("##toolbar", nullptr, flags))
    {
        ...
        ImGui::SameLine(0, 20);
        ImVec4 col =
            s.error_occured ? ImVec4(1.0f, 0.3f, 0.3f, 1.0f) : ImVec4(0.5f, 1.0f, 0.5f, 1.0f);
    ...
    if (ImGui::Begin("Register File", nullptr, flags))
    {
                              ...
                              ImVec2(0, H - 40)))
        {
            ...
            for (int i = 0; i < 32; ++i)
            {
                ...
                // The registers that will change will be yellow in color - got the color scheme
                // from the pictures in the IDE readme in pegasus
                ImVec4 col;
                ...
                else if (s.reg_changed[i])
                    col = {1.0f, 1.0f, 0.2f, 1.0f};
...
int main(int argc, char** argv)
{
    ...
    SDL_Window* window = SDL_CreateWindow(
        "Pegasus CoSim Debugger", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1600, 900,
    ...
    while (running)
    {
        ...
        if (state.step_automatically && state.running && !state.finished)
            walk(state);
